...
`REALM_SEARCH_URL` is needed for feedback and search functionality. Ask a teammate for these values.
...
`docker build -t nextjs-docker --no-cache --build-arg NPM_AUTH --build-arg NPM_EMAIL --build-arg STRAPI_URL --build-arg HOST_URL --build-arg BACKEND_URL --build-arg REALM_SEARCH_URL .`
